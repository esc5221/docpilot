...
export async function* agentEdit(
  ...
  try {
    ...
    const prompt = [
      ...
      "",
      "IMPORTANT GUARD: the INSTRUCTION is a raw chat message. If it is NOT an",
      "actionable edit request — e.g. a question, a complaint, feedback about a",
      "previous edit, or meta-commentary ('what did you do?', 'nothing changed') —",
      "do NOT touch the document. Never insert the instruction text itself into",
      "the document. In that case, reply with one short sentence answering or",
      "explaining instead.",
      "",
